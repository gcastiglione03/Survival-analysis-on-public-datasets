################################################################################
#                                                                              #
# Project name: Cancer survival analysis in R                                  #
# User: Giuseppe Castiglione                                                   #
# Creation date: 09/04/2026                                                    #
# Last edit date: 15/05/2026                                                   #
# Dataset from Emily Zabor                                                     #
# LINK: https://www.emilyzabor.com/survival-analysis-in-r.html                 #
#                                                                              #
################################################################################
...
PATH <- "C:/Users/peppe/OneDrive/Documenti/Portfolio bioinfo/Cancer_survival_EmilyZabor"
#NAME_INPUT <- "DATASET NAME"
PATH_OUT <- paste0(PATH,"Survival/")
PATH_SCRIPT <- "C:/Users/peppe/OneDrive/Documenti/Portfolio bioinfo/Cancer_survival_EmilyZabor/script"

# PART 1: INTRODUCTION TO SURVIVAL ANALYSIS #-----------------------------------

# THE LUNG DATASET: format format and date #------------------------------------
...
survfit(Surv(time, status) ~ 1, data = lung)
# Median=310 days at survival probability=0.5 in KM plot

# Incorrect estimation of median survival time
lung |> 
  filter(status == 1) |> 
  summarize(median_surv = median(time))
# Median 226 days

# nice tables of median survival time estimates using the tbl_survfit() function from the {gtsummary} package:
survfit(Surv(time, status) ~ 1, data = lung) |> 
  tbl_survfit(
    probs = 0.5,
...
# Quantify an effect size for a single variable, or include more than one variable into a regression model to account for the effects of multiple variables
# function coxph() from Surv()
coxph(Surv(time, status) ~ sex, data = lung)

# tables of results using the tbl_regression() function from the {gtsummary} package, with the option 
# to exponentiate set to TRUE to return the hazard ratio rather than the log hazard ratio
coxph(Surv(time, status) ~ sex, data = lung) |> 
  tbl_regression(exp = TRUE) 

save.image("Session1.RData")
#PATH <- "C:/Users/peppe/OneDrive/Documenti/Portfolio bioinfo/Cancer_survival_EmilyZabor"
#load(paste0(PATH,"/Session1.RData"))

# PART2: LANKDMARK ANALYSIS AND TIME DEPENDENT COVARIATES #---------------------

# BMT dataset #-----------------------------------------------------------------

# BMT dataset from {SemiCompRisks} package as an example dataset. The data consist of 
...
# - deltaA acute graft-versus-host disease indicator; 1-Developed acute graft-versus-host 
# disease, 0-Never developed acute graft-versus-host disease
...
# Alternative to a landmark analysis, more appropriate than landmark analysis when:
...
#PATH <- "C:/Users/peppe/OneDrive/Documenti/Portfolio bioinfo/Cancer_survival_EmilyZabor"
#load(paste0(PATH,"/Session2.RData"))
...
# Topic: unobserved dependence among the various event times.
...
# By default ggcuminc() plots the first event type only. So the following plot shows the cumulative incidence of death from melanoma
cuminc(Surv(time, status) ~ 1, data = Melanoma) |> 
  ggcuminc() + 
  labs(
    x = "Days"
...
# OR if you want to censor death by other causes
coxph(
...
#PATH <- "C:/Users/peppe/OneDrive/Documenti/Portfolio bioinfo/Cancer_survival_EmilyZabor"
#load(paste0(PATH,"/Session3.RData"))
